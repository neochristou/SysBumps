...
inline void __attribute__((always_inline)) maccess(void *p) { asm volatile("movq (%0), %%rax\n" : : "c"(p) : "rax"); }
...
static inline __attribute__((always_inline)) size_t rdtsc_begin(void) {
  size_t a, d;
  asm volatile("mfence");
  asm volatile("rdtsc" : "=a"(a), "=d"(d));
  a = (d << 32) | a;
...
static inline __attribute__((always_inline)) size_t rdtsc_end(void) {
  size_t a, d;
  asm volatile("lfence");
  asm volatile("rdtsc" : "=a"(a), "=d"(d));
  a = (d << 32) | a;
...
#define ESET_L1_OFFSET (L1_DTLB_SETS * PAGE_4K)
#define ESET_L1_SIZE L1_DTLB_WAYS
#define ESET_L2_OFFSET (L2_TLB_SETS * PAGE_4K)
// Original ESET_SIZE = 12
#define ESET_L2_SIZE L2_TLB_WAYS

void gen_eset_sysbumps(size_t target, void **eset_l2, void **eset_l1,
                       size_t flush_base) {
  size_t base_l1 = flush_base + ((uint64_t)target & (ESET_L1_OFFSET - 1));
  for (int i = 0; i < ESET_L1_SIZE; i++) {
    eset_l1[i] = (void *)(base_l1 + (ESET_L1_OFFSET + 128) * (i + 1));
  }

  size_t base_l2 = flush_base + ((uint64_t)target & (ESET_L2_OFFSET - 1));
  for (int i = 0; i < ESET_L2_SIZE; i++) {
    eset_l2[i] = (void *)(base_l2 + (ESET_L2_OFFSET + 128) * (i + 1));
  }
  memory_fence();
}
